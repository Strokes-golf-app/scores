...
the top match -- so one call can load several courses at once. If a
search result already includes hole-level detail (tees/holes), the
script uses it directly with zero extra calls. Only when a result is
missing that detail does the script spend a second call on
GET /v1/courses/{id} to fetch it.
...
def api_search(term):
    ...
    if status != 200 or not isinstance(data, dict):
        return []
    return data.get("courses") or data.get("results") or []
...
def api_get_course(course_id):
    ...
    return data.get("course") if isinstance(data.get("course"), dict) else data
        ...
        for term in terms:
            ...
            for course in results:
                ...
                if not holes:
                    ...
                    calls_used += 1
                    if detail is None:
                        print(f"    \"{name}\": detail fetch failed — skipping")
                        progress["courses"][key] = {"status": "error", "reason": "detail fetch failed", "at": now_iso()}
                        summary["errors"] += 1
                        save_progress(progress)
                        continue
